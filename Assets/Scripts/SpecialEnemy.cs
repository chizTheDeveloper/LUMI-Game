...
public class SpecialEnemy : MonoBehaviour
{
    ...
    void Update()
    {
       ...
       if(gameObject.GetComponent<SpecialHealth>().currentHealth <= 0){
         ...
         key = (GameObject)Instantiate(Resources.Load("FinalKey"));
         key.transform.position = lastPos;
         ...
         questTracker = playerGO.GetComponent<QuestTracker>();
            if(!questTracker.progress.Contains("foundKey")){
                questTracker.progress.Add("foundKey");
                questTracker.interactingWith = "companion";
            }
